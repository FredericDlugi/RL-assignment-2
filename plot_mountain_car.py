...
def save_visits(folder):
    visit_counts = np.load(os.path.join("mountain_car_results", folder, "visits.npy"))
...
def save_policy(folder):
    learned_policy = np.load(os.path.join("mountain_car_results", folder, "learned_policy.npy"))

    indicies_1 = np.argwhere(learned_policy == 1)
    plt.scatter(indicies_1[:,0], indicies_1[:,1], marker=".", label="no push")
    indicies_0 = np.argwhere(learned_policy == 0)
    plt.scatter(indicies_0[:,0], indicies_0[:,1], marker="<", label="push left")
    indicies_2 = np.argwhere(learned_policy == 2)
    plt.scatter(indicies_2[:,0], indicies_2[:,1], marker=">", label="push right")
...
if __name__ == "__main__":
    ...
    folder = "sarsa"
    save_policy(folder)
    sarsa_episodic_returns = np.load(os.path.join("mountain_car_results", folder, "episodic_returns.npy"))
    save_visits(folder)

    folder = "q_learning"
    save_policy(folder)
    qlearn_episodic_returns = np.load(os.path.join("mountain_car_results", folder, "episodic_returns.npy"))
    save_visits(folder)
    ...
    generations = np.arange(sarsa_episodic_returns.shape[0])
    plt.scatter(generations, sarsa_episodic_returns, label="SARSA")
    plt.scatter(generations, qlearn_episodic_returns, label="Q-Learning")
    plt.legend()
